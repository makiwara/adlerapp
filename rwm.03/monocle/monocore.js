...
Monocle.Factory = function (element, label, index, reader) {
  ...
  function prepend(tagName, oLabel, indexOrOptions, orOptions) {
    var oElement = make.apply(this, arguments);
    p.element.insertBefore(oElement,p.element.childNodes[0]);
    return oElement;
  }
...
Monocle.Panels.Magic = function (flipper, evtCallbacks) {
  ...
  function startListening() {
    ...
    $(p.parts.reader.dom.find('page')).find('iframe').each(function(){
        p.startListeners.push([
          this.contentDocument.defaultView,
          Monocle.Events.listenForContact(
            this.contentDocument.defaultView,
            { 'start': translatorFunction(this, cmptContactStart) }
          )
      ]);
    })
    /*
    for (var i = 0, ii = p.parts.cmpts.length; i < ii; ++i) {
      console.log(p.parts.cmpts[i])
      p.startListeners.push([
  ...
  function listenForMoveAndEnd(fnMove, fnEnd) {
    ...
    $(p.parts.reader.dom.find('page')).find('iframe').each(function(){
      listenOnElem(
        this.contentDocument.defaultView,
        translatorFunction(this, fnMove),
        translatorFunction(this, fnEnd)
      );
    })
    /*
    for (var i = 0, ii = p.parts.cmpts.length; i < ii; ++i) {
...
Monocle.Dimensions.Columns = function (pageDiv) {
  ...
  function setColumnWidth() {
    ...
    var rules = [
      ...
      '}',
      '.CHAT { background: rgba(255,0,0,.25) }'
    ]
  ...
  API.pageDimensions = pageDimensions;
